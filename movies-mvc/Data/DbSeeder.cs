...
namespace movies_mvc.Data
{
    public class DbSeeder
    {
        ...
        public static async Task SeedAsync(MovieDbContext context/*, UserManager<Usuario> userManager, RoleManager<IdentityRole> roleManager*/)
        {
            ...
            var plataformas = new List<Plataforma>
    {
        ...
        new Plataforma {
            ...
            LogoUrl = "https://upload.wikimedia.org/wikipedia/commons/9/90/Prime_Video_logo_%282024%29.svg"
        },
        new Plataforma {
            Nombre = "Disney+",
            ...
            var peliculas = new List<Pelicula>
    {
        ...
        // Remove seeded data (useful for tests / resetting the database)
        public static async Task RemoveSeedAsync(MovieDbContext context)
        {
            if (context == null) throw new ArgumentNullException(nameof(context));

            // Remove dependent entities first to satisfy FKs
            ...
            if (favoritos.Count > 0) context.Favoritos.RemoveRange(favoritos);

            var peliculas = await context.Peliculas.ToListAsync();
            if (peliculas.Count > 0) context.Peliculas.RemoveRange(peliculas);

            await context.SaveChangesAsync();

            var generos = await context.Generos.ToListAsync();
            if (generos.Count > 0) context.Generos.RemoveRange(generos);

            var plataformas = await context.Plataformas.ToListAsync();
            if (plataformas.Count > 0) context.Plataformas.RemoveRange(plataformas);
